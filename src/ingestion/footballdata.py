"""Étage 1 — Ingestion football-data.co.uk (Ligue 1).

Télécharge les CSV de résultats + cotes de la Ligue 1 (code `F1`) et les écrit
dans data/raw/footballdata/ en respectant l'IMMUTABILITÉ de la couche raw
(cf. ARCHITECTURE.md §0 et discipline ELT raw/staging/marts) :

  - Chaque téléchargement = un fichier horodaté en UTC, JAMAIS réécrit.
    Ex. F1_2526_20260531T143005Z.csv
...
# Code football-data de la Ligue 1.
LEAGUE_CODE = "F1"
...
_USER_AGENT = "ligue1-betting/0.1 (ingestion footballdata)"
...
def recent_seasons(n: int = 5, today: datetime | None = None) -> list[str]:
    ...
    Une saison de Ligue 1 va d'août à mai. Convention football-data : la saison
    2025-26 est codée '2526'. On considère qu'une saison "démarre" en juillet :
    avant juillet, la saison courante est (année-1)/année.
    """
