...
Journal séparé du 1/N/2 (choix : deux marchés analysés indépendamment, dynamiques
de clôture différentes). Même discipline que betlog.py : cote précoce conservée,
dédup, append-only, immuable.

Identité d'un pari over/under = (date_match, home, away, seuil, sens) — un même
match peut générer des paris sur plusieurs seuils, distincts.

Journal : data/bets/bet_log_ou.csv.
"""
...
FIELDS = [
    ...
    "threshold", "side",       # seuil (2.5…) et sens (over/under)
...
def load_log_ou(path: Path = DEFAULT_LOG_OU) -> list[OUBetRecord]:
    path = Path(path)
    if not path.exists():
        return []
    out = []
...
def existing_identities_ou(path: Path = DEFAULT_LOG_OU) -> set:
    return {r.identity for r in load_log_ou(path)}


def record_bets_ou(
    ou_match_scores: list,                       # list[OUMatchScore]
    match_dates: dict[tuple[str, str], str],     # (home,away) -> date match
    threshold: float = DEFAULT_SCORE_THRESHOLD,
    log_path: Path = DEFAULT_LOG_OU,
    now: datetime | None = None,
) -> list[OUBetRecord]:
    """Inscrit les paris over/under dont le score dépasse le seuil, sans doublon."""
    log_path = Path(log_path)
    log_path.parent.mkdir(parents=True, exist_ok=True)
    stamp = (now or datetime.now(timezone.utc)).strftime("%Y%m%dT%H%M%SZ")
    seen = existing_identities_ou(log_path)

    new_records: list[OUBetRecord] = []
