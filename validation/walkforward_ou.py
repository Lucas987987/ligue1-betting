...
def run(matches_path: Path = DEFAULT_MATCHES, out_dir: Path = DEFAULT_OUT):
    ...
    print("\n=== VERDICT ===")
    worst = max(THRESHOLDS, key=lambda s: _max_calib_gap(model_m[s].calib))
    for s in THRESHOLDS:
        gap = _max_calib_gap(model_m[s].calib)
        verdict = "bien calibré" if gap < 0.06 else "DÉCALÉ — prudence"
        print(f"  Over/Under {s} : écart calib max {gap:.3f} → {verdict}")
