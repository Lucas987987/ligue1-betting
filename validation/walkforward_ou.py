...
quand le modèle dit "64 % d'over 2.5", y a-t-il ~64 % d'overs réels sur ces
matchs ? La vérité de terrain est nette : total réel = FTHG + FTAG, comparé au
seuil (demi-entier, pas d'égalité).

Le modèle est déjà calibré sur le 1/N/2 ; ça NE garantit PAS qu'il l'est sur les
totaux, surtout aux seuils bas (0.5, 1.5) où la correction Dixon-Coles pèse. On
mesure, on ne suppose pas.

Comparaison au marché : football-data ne fournit des cotes que pour le seuil 2.5
(P>2.5 / P<2.5 = Pinnacle clôture ; Avg>2.5 / Avg<2.5 = moyenne marché). Donc on
compare au marché UNIQUEMENT sur 2.5 ; les autres seuils n'ont pas de référence.

Protocole : même walk-forward que le 1/N/2 (amorçage 2 saisons, modèle entraîné
sur le passé strict de chaque date). On utilise le modèle FRÉQUENTISTE de base
pour la vitesse (le bayésien donnerait des points quasi identiques en moyenne ;
ici on valide la calibration de la matrice de scores, pas l'incertitude).
"""
...
# Colonnes de clôture over/under 2.5 dans football-data.
OU_PINNACLE = ("PSCH_placeholder",)  # non utilisé ; on lit directement ci-dessous
COL_P_OVER, COL_P_UNDER = "P>2.5", "P<2.5"          # Pinnacle clôture
COL_AVG_OVER, COL_AVG_UNDER = "Avg>2.5", "Avg<2.5"  # moyenne marché (repli)


def _over_probs(params, home: str, away: str) -> dict:
    """P(over S) par seuil, depuis le modèle de base (matrice DC)."""
...
def _market_over_25(row) -> float | None:
    """Proba over 2.5 dévigorisée depuis les cotes clôture (Pinnacle, repli moy)."""
...
class OUMetrics:
    """Accumule log-loss, Brier et bins de calibration pour un seuil."""
    def __init__(self):
        self.ll = 0.0
        self.brier = 0.0
        self.n = 0
        self.calib = []  # (proba over prédite, over réalisé 0/1)

    def add(self, p_over: float, over_real: int):
        p = min(max(p_over, EPS), 1 - EPS)
        self.ll += -(over_real * math.log(p) + (1 - over_real) * math.log(1 - p))
        self.brier += (p - over_real) ** 2
        self.calib.append((p, over_real))
        self.n += 1
...
def run(matches_path: Path = DEFAULT_MATCHES, out_dir: Path = DEFAULT_OUT):
    ...
    # On a besoin des lignes brutes pour les cotes O/U → relecture CSV indexée.
    raw_by_key = {}
    ...
    market_m = OUMetrics()  # seuil 2.5 uniquement
    ...
    for s in THRESHOLDS:
        mm = model_m[s]
        print(f"\n--- Over/Under {s}  (log-loss {mm.log_loss:.4f}, "
              f"Brier {mm.brier_score:.4f}) ---")
    ...
    print(f"  Écart : {gap:+.4f}  "
          + ("(le modèle s'approche du marché)" if gap > 0
             else "(le modèle bat le marché — rare, à vérifier)"))

    print("\n=== VERDICT ===")
    print("  (ECE = écart moyen pondéré par effectif, la mesure d'ensemble ;")
    print("   max = pire tranche, repère un défaut localisé)")
    for s in THRESHOLDS:
        ece = _ece(model_m[s].calib)
        gap = _max_calib_gap(model_m[s].calib)
        # Verdict sur l'ECE : <0.03 excellent, <0.05 bon, sinon prudence.
        if ece < 0.03:
            verdict = "bien calibré"
        elif ece < 0.05:
            verdict = "acceptable"
        else:
            verdict = "DÉCALÉ — prudence"
        note = ""
        if ece < 0.05 <= gap:
            note = "  (défaut localisé sur une tranche, ensemble OK)"
...
def _max_calib_gap(calib, n_bins=10):
    """Plus grand écart |prédit − réel| sur les bins à effectif suffisant (>=20).
    Utile pour repérer un problème LOCALISÉ, mais sensible au bruit des petits bins."""
...
def _ece(calib, n_bins=10):
    """Expected Calibration Error : écart |prédit − réel| moyen PONDÉRÉ par
    l'effectif de chaque tranche. C'est la mesure d'ensemble de référence : une
    tranche à 340 matchs pèse beaucoup, une à 4 matchs presque rien. Insensible
    au bruit des petites tranches, contrairement à l'écart max."""
    rows = _calib_table(calib, n_bins)
...
def m_date_str(m):
    """Reformate la date d'un Match au format football-data dd/mm/YYYY."""
