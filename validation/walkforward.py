...
class Metrics:
    ...
    brier_sum: float = 0.0
    # pour la calibration : (proba prédite issue réalisée, issue gagnante 0/1)
    ...
    def add(self, probs: dict[str, float], result: str):
        ...
        # Brier multiclasse : somme (p_i - y_i)^2 sur les 3 issues.
        for k in ("home", "draw", "away"):
            y = 1.0 if {"home": "H", "draw": "D", "away": "A"}[k] == result else 0.0
            self.brier_sum += (p[k] - y) ** 2
        self.calib.append((p[key], 1))
        self.n += 1
